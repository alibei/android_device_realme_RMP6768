...
constexpr char kTypecPath[] = "/sys/class/dual_role_usb/"; // We have /sys/class/dual_role_usb, not /sys/class/typec
constexpr char kDataRoleNode[] = "/data_role";
constexpr char kPowerRoleNode[] = "/power_role";
constexpr char kModeNode[] = "/mode"; // We have a separate file for mode
...
string appendRoleNodeHelper(const string &portName, PortRole::Tag tag) {
    ...
    switch (tag) {
        ...
        case PortRole::mode:
            return node + kModeNode; // We have a separate file for mode
        default:
            return node + kModeNode; // Disassembly shows no separate case for non-matches and MODE case could be removed, leaving just default, but let's keep it like that for now in case we need to change something.
    }
}
...
string convertRoletoString(PortRole role) {
    if (role.getTag() == PortRole::powerRole) {
        ...
        if (role.get<PortRole::mode>() == PortMode::UFP)
            return "ufp"; // We probably call these 'ufp' and 'dfp', as seen in switchToDrp for example
    ...
    */
    for (int i = 0; i < 20; ++i) {
        ALOGI("filename write: %s role:%s", filename.c_str(),
          convertRoletoString(in_role).c_str());
...
Status getCurrentRoleHelper(const string &portName, bool connected, PortRole *currentRole) {
    ...
    if (currentRole->getTag() == PortRole::powerRole) {
        ...
        filename = kTypecPath + portName + kModeNode; // We have a separate file for setting mode.
...
bool canSwitchRoleHelper() {
    string filename = "/sys/class/tcpc/type_c_port0/pe_ready";
    string supportsPD;
    ...
    if (result == Status::SUCCESS) {
        ...
        for (std::pair<string, bool> port : names) {
            ...
            (*currentPortStatus)[i].canChangeMode =
                port.second ? canSwitchRoleHelper() : false;
                ...
                for (unsigned long i = 0; i < currentPortStatus.size(); i++) {
                    DIR *dp =
                        opendir(string(kTypecPath +
                                       string(currentPortStatus[i].portName.c_str())).c_str()); // Stock HAL did not append -partner. Based on disassembly - 0x65E0 in android.hardware.usb@1.1-service-mediatek from V12.0.1.0.QJOEUXM
                    if (dp == NULL) {
